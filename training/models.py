...
class MultimodalTrainer:
    def __init__(self,model,train_loader,val_loader):
        ...
        # ============= UNCOMMENT THIS BLOCK IF BELOW NOT WORKING =============
        
        # timestamp = datetime.now().strftime('%b%d_%H-%M-%S') #Dec 01_12-30-45
        ...
        # Check if we are in SageMaker
        if 'SM_TRAINING_ENV' in os.environ:
            # Use the EXACT path defined in train_sagemaker.py
            log_dir = '/opt/ml/output/tensorboard'
        else:
            # Local development path
            timestamp = datetime.now().strftime('%b%d_%H-%M-%S')
            log_dir = f"runs/run_{timestamp}"
        ...
        # Very high : 1 , High : 0.1-0.01 , Moderate : 1e-1 , Low : 1e-4, Very low : 1e-5
        ...
        print(f"Sentiments Weigths on device: {self.sentiment_weights.device}")
    ...
    def train_epoch(self):
        ...
        for batch in self.train_loader:
            ...
            # Graident clipping to prevent exploding gradients
        ...
        # Compute the precision and accuracy for the validation set
    ...
    def evaluate(self,data_loader,phase="val"):
        ...
        # Return average losses and predictions for the validation set
